...
The Evidence Matrix names dmrseq as the primary genome-wide DMR method, caveat
*"paired design and covariates must be encoded correctly"*. Iteration 51's Arm C
found dmrseq **failed on all 48 array tasks at `BSseq()`**, so chr6 was never
region-tested. Those two facts belong on one node, and a reader of either file
alone gets a misleading picture.
