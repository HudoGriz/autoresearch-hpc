...
**Orphan nodes.** A node the tables describe and the graph never connects. For example, a
variant node can sit near the region it is argued to explain with no edge
between them, so the claim rests on an adjacency that does not exist. `dl dag check` fails on this.
